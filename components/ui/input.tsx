...
function Input({ className, min, type, onKeyDown, onWheel, ...props }: React.ComponentProps<"input">) {
  ...
  return (
    <input
      ...
      className={cn(
        "file:text-foreground placeholder:text-muted-foreground selection:bg-primary selection:text-primary-foreground dark:bg-input/30 border-input flex h-10 max-h-10 w-full min-w-0 items-center rounded-xl border bg-white px-4 py-2 text-[13px] shadow-sm transition-[color,box-shadow] outline-none file:inline-flex file:items-center file:h-6 file:border-0 file:bg-transparent file:text-[13px] file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50",
        "focus-visible:border-primary focus-visible:ring-primary/20 focus-visible:ring-4",
        "aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
        className
      )}
