...
export const GET = withAuth(async (request: NextRequest, user) => {
  try {
    ...
    const quotations = await mongoStore.getAll("quotations", { userId: user.id }, { skip, limit, sort: { date: -1 } })
    const total = await mongoStore.count("quotations", { userId: user.id })
...
export const POST = withAuth(async (request: NextRequest, user) => {
  try {
    ...
    if (!body.quotationNumber) {
      body.quotationNumber = await generateNextNumber("quotations", "QT", user.id)
    }

    const quotation = await mongoStore.create("quotations", { ...body, userId: user.id })

    await logActivity(user.id, "create", "quotation", quotation._id?.toString(), {
