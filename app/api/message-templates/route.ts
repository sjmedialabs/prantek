...
export const PUT = withAuth(async (request: NextRequest, user) => {
  try {
    ...
    await db.collection(Collections.MESSAGE_TEMPLATES).updateOne(
      { _id: new ObjectId(id) }, { $set: { ...update, updatedAt: new Date() } }
    )
    const updated = await db.collection(Collections.MESSAGE_TEMPLATES).findOne({ _id: new ObjectId(id) })
...
export const DELETE = withAuth(async (request: NextRequest, user) => {
  try {
    ...
    if (!id) return NextResponse.json({ success: false, error: "ID required" }, { status: 400 })
    await db.collection(Collections.MESSAGE_TEMPLATES).deleteOne({ _id: new ObjectId(id) })
