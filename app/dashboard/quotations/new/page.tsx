...
export default function NewQuotationPage() {
  ...
  const [quotationItemTotal, setQuotationItemTotal] = useState(0)
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const loadedTaxRates = await api.taxRates.getAll()
        console.log("LOADED ITEMS ARE", loadedItems)
        const companyData = await api.company.get()
        const items = loadedItems.filter((eachItem: any) => eachItem.isActive === true)
  ...
  const updateItem = (id: string, field: keyof QuotationItem, value: string | number) => {
    ...
    setItems(
      items.map((item) => {
        ...
        setQuotationSubtotal(item.amount)
        setQuotationTaxAmount(item.taxAmount)
        setQuotationItemTotal(item.total)
        return item
      }),
    )
  }
    ...
    try {
      const quotationData = {
        ...
        items: items.map((item) => ({
          type: item.type,
          itemName: masterItems.find((m) => m._id === item.itemId)?.name || "",
  ...
  clients.some(
    (c) =>
      c?.email?.toLowerCase() === newClient.email.trim().toLowerCase()
  );
...
if (emailExists) {
  ...
  isValid = false;
    ...
    // if (newClient.city && !newClient.city.trim()) {
            ...
            <TabsContent value="client" className="mt-6">
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div>
                    <Label htmlFor="clientSelect" required>Client Name</Label>
                  ...
                  <div>
                    <Label htmlFor="clientAddress" required>Address</Label>
                    <Textarea
                      id="clientAddress"
                      value={clientAddress}
                      ...
                      className={isClientSelected ? "bg-gray-100 cursor-not-allowed" : ""}
                    />
                    <p className="text-xs text-gray-500 mt-1">Auto-populated from client record</p>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="clientContact" required>Contact Number</Label>
                      <Input
                        id="clientContact"
                        value={clientContact}
